...
const Projects = () => {
  ...
  const fetchProjects = async () => {
    try {
      const res = await apiConnector('GET', projectEndpoint.GETALL_PROJECT_API);
      console.log(res.data);
      const myArray = Object.values(res.data.data);
      console.log(myArray,"projrctsS")
       setProjects(myArray);
  ...
  return (
   ...
   {projects.map((project, index) => (
    ...
    whileHover={{
      ...
      <div className="p-5">
        <a href="#">
          <h5 className="mb-2 text-3xl font-base3 tracking-tight text-gray-900 dark:text-white">{project.title}</h5>
        </a>
